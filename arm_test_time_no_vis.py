...
def combined_dist_func(network,pc,jps,normalize=False,verbose=False):
    ...
    poses,meshes,_=util_no_vis.generate_poses_from_jps(robot,[jps],active_link_names,active_joint_names,sizes)
...
def trimesh_dist_func(pc,jps,rot_dir=False):
    ...
    if rot_dir:
        ...
        rot_dir=torch.tensor(np.array(util_no_vis.rot_dir_from_point(torch.tensor(p).type(torch.float),torch.tensor(p_init).type(torch.float),eul_rot))).type(torch.float)
    else:
...
def path_adjust(scene,jps,dist_func,motion_scale=0.02,dist_threshold=0.02,scale_adjust=1,max_tries=200):
    ...
    test_pose,_,_=util_no_vis.generate_poses_from_jps(robot,[final_jps],active_link_names,active_joint_names,sizes)
    
    goal_ee_position=test_pose[0,-1,:3]

    scene_meshes,ta=util_no_vis.create_meshes(scene)
    pc=util_no_vis.create_pc(scene_meshes,5000,ta)
...
def get_grad(pc,dist_func,jps):
    ...
    nogpc=util_no_vis.gen_local_pc(robot,active_link_names,active_joint_names,sizes,jps,pc)
...
def check_goal(jps_i,jps_f):
    ...
    test_pose,_,_=util_no_vis.generate_poses_from_jps(robot,[jps_i,jps_f],active_link_names,active_joint_names,sizes)
...
for i in range(3,4):
    ...
    scene_meshes,_=util_no_vis.create_meshes(scene)
    scene_objs=[]
    ...
    test_poses,test_meshes,test_transforms=util_no_vis.generate_poses_from_jps(robot,jps_path_4,active_link_names,active_joint_names,sizes)
    ...
    for link_meshes in test_meshes:
        ...
        for link_mesh in link_meshes:
            ...
            obj=fcl.CollisionObject(geom)
            md,_,point2,_,_=util_no_vis.calculate_distance(scene_objs,obj,penetration_depth=True)
...
grm_net_3=util_no_vis.create_grad_rot_matricies(pos_grad_dirs,closest_points_net_4)
grm_tm_3=util_no_vis.create_grad_rot_matricies(tmgds_4,closest_points_tm_4)


test_traj_8=util_no_vis.generate_poses_from_jps(robot,jps_path_4,active_link_names,active_joint_names,sizes)
